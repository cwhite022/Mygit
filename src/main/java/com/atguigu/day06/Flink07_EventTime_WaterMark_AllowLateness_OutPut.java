 ...
 * @Version 1.0
 */
public class Flink07_EventTime_WaterMark_AllowLateness_OutPut {
...
//        DataStreamSource<String> streamSource = env.readTextFile("input/sensor.txt");
        ...
        WindowedStream<WaterSensor, Tuple, TimeWindow> window =
                ...
                //设置允许迟到的数据（实质就是推迟关窗的时间）
                .allowedLateness(Time.seconds(4))
                //TODO 将关窗迟到后的数据放到测输出流中
                .sideOutputLateData(outputTag)
                ;

        SingleOutputStreamOperator<String> process = window.process(new ProcessWindowFunction<WaterSensor, String,
                Tuple, TimeWindow>() {
            @Override
