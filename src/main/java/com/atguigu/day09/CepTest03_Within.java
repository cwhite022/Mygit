 ...
 *
 * 需求：以WaterSensor 数据
 *      如果同一个传感器10s内连续3条水位线超过30 ，则输出报警信息(那个传感器水位线连续3次超过30)
 *  CEP编程 3 步：
 *      1.定义模式序列（规则）
 *      2.将模式顺序作用到流上
 *      3.提取事件（注意：如果使用了Within关键字，需要考虑是否提取超时事件---->超时事件是放在测输出流的）
 */
public class CepTest03_Within {
        ...
        SingleOutputStreamOperator<String> selectDS = patternStream.select(outputTag,
                new PatternTimeoutFunction<WaterSensor, String>() {
                    ...
                    public String timeout(Map<String, List<WaterSensor>> map, long l) throws Exception {
                        ...
                        System.out.println("***********************TimeOut***********************");
                        return start.get(0).getId() + "存在" + map.size() + "条数据!";

                    }
                }, new PatternSelectFunction<WaterSensor,
                        String>() {
                    @Override
